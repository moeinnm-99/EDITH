...
try:
    while True:
        ...
        elif 'brightness' in data or 'screen brightness' in data:
            try:
                speak("Should I increase or decrease brightness?")
                action = command().lower()
                if 'increase' in action:
                    pyautogui.press('brightnessup')
                    speak("Increased screen brightness")
                elif 'decrease' in action:
                    pyautogui.press('brightnessdown')
                    speak("Decreased screen brightness")
                else:
                    speak("Please say increase or decrease brightness")
            except Exception as e:
                speak("Sorry, I couldn't adjust brightness")

        # Play YouTube video
        elif 'play video' in data or 'play on youtube' in data:
            speak("What video should I play?")
            video = command().lower()
            wb.open_new(f'https://www.youtube.com/results?search_query={video}')
            speak(f"Playing {video} on YouTube")

        # Search images
        elif 'search images' in data or 'find pictures' in data:
            speak("What images should I search for?")
            images = command().lower()
            wb.open_new(f'https://www.google.com/search?tbm=isch&q={images}')
            speak(f"Showing image results for {images}")

        # Unit conversion
        elif 'convert' in data:
            speak("What unit conversion should I do? For example: 10 inches to cm")
            conversion = command().lower()
            try:
                res = client.query(conversion)
                results = next(res.results).text
                speak(f'{conversion} is {results}')
            except:
                speak("Sorry, I couldn't perform this conversion")

        # Get news headlines
        elif 'news' in data or 'headlines' in data:
            speak("Getting today's headlines")
            wb.open_new('https://news.google.com')
            speak("Here are today's top stories")

        # Simple workout suggestions
        elif 'workout' in data or 'exercise' in data:
            exercises = [
                "How about 20 pushups?",
                "Try doing 3 sets of 15 squats",
                "Let's do 1 minute plank",
                "Jumping jacks for 2 minutes would be great",
                "How about some stretching exercises?"
            ]
            speak(random.choice(exercises))

        # Set reminders
        elif 'remember that' in data:
            speak("What should I remember?")
            rem = command()
            speak("When should I remind you? (For example: 15:30)")
            rem_time = command()
            try:
                reminders[rem_time] = rem
                speak(f"I'll remind you at {rem_time} about {rem}")
            except:
                speak("Sorry, I couldn't set that reminder")
            # Here you would add actual reminder functionality using time and notifications
        ...
        elif "shut down" in data or "logout" in data or "restart" in data or "turn off" in data:
        ...
        # exits the program ************-----------------******************----------------***********************
        ...
        elif "offline" in data or "goodbye" in data or "bye" in data or "bye bye" in data or "good bye" in data:
            if currentTime.hour < 17:
                ...
                pass
            speak('EDITH powering off in 3, 2, 1')
            exit()
